...
def combine(temp_dat, term_type):
    term_dict = {
        "mass": (WORD_MASS, WORD_BOND, 3),
        "bond": (WORD_BOND, WORD_ANGLE, 5),
        "angle": (WORD_ANGLE, WORD_DIHE, 8),
        "dihedral": (WORD_DIHE, WORD_IMPROPER, 11),
        "improper": (WORD_IMPROPER, WORD_NONBON, 11),
        "nonbond": (WORD_NONBON, WORD_NEWLINE, 5)
    }

    start_word, end_word, b = term_dict[term_type]

    with open(temp_dat) as inFile, open(letters_from_frcmod, "w") as outFile:
        buffer = []
        for line in inFile:
            if line.startswith(start_word):
                buffer = ['']
    ...
    term_letters = []
    with open(term_letters_file, "w") as f:
        for i in range(len(x)):
            n = x[i][:b]
            m = x[i][b:]
            p = n + m
...
term_types = ["mass", "bond", "angle", "dihedral", "improper", "nonbond"]
file_paths = ["info/non_zero_check.dat", "info/non_zero_temp.dat", "info/desired.frcmod"]

combined_terms = {term_type: [] for term_type in term_types}

for term_type in term_types:
    for file_path in file_paths:
        combined_terms[term_type].extend(combine(file_path, term_type))
